...
*/}}
{{- define "mysql.headlessSvc" -}}
{{- printf "mysql-headless" }}
{{- end }}
...
Used on PRIMARY ONLY (which has net1). Secondaries have no MacVLAN so this
init container is still safe to run on them — it exits immediately when net1
is absent (after 30 × 2s timeout it proceeds without error).

Fixes applied:
  1. Wait for net1 (primary) or timeout gracefully (secondaries)
...
- name: fix-routing
  ...
  command:
    ...
    - |
      ...
      if [ "$NET1_UP" = "0" ]; then
        echo "[fix-routing] net1 not present (secondary pod) — applying eth0 routes only"
      fi
...
{{/*
═══════════════════════════════════════════════════════════════════════════════
consul-tag-patcher init container — PRIMARY ONLY
═══════════════════════════════════════════════════════════════════════════════
...
Design — identical to Redis/Kamailio pattern:
  • connect-inject: true  → Envoy sidecar. Health checks over eth0 (pod IP).
  • transparent-proxy: false → MySQL TCP ports bypass Envoy entirely.
  • No direct Consul API calls — Envoy owns service lifecycle.
  • consul.hashicorp.com/service-meta-* annotations always land in Consul.
  • Safety guard: refuses to write 10.x.x.x / 172.x.x.x to Consul
    (ensures only MacVLAN IP 192.168.x.x is registered as multus_ip).

Args: .nadName .port .role .ctx
*/}}
{{- define "mysql.consulTagPatcher" -}}
{{- $nadName := .nadName -}}
{{- $port    := .port    -}}
{{- $role    := .role    -}}
{{- $ctx     := .ctx     -}}
- name: consul-tag-patcher
  ...
  command:
    ...
    - |
      ...
      POD_PORT = os.environ.get("POD_PORT", "3306")
      ...
      patch = json.dumps({"metadata": {"annotations": {
          ...
          "mysql.agnoshin.io/multus-ip":   MULTUS_IP,
          "mysql.agnoshin.io/role":        POD_ROLE,
          "mysql.agnoshin.io/registered":  "true",
      }}}).encode()
