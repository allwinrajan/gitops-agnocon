...
*/}}
{{- define "superset-service.fullname" -}}
...
Common labels
*/}}
{{- define "superset-service.labels" -}}
helm.sh/chart: {{ include "superset-service.chart" . }}
{{ include "superset-service.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
Selector labels
*/}}
{{- define "superset-service.selectorLabels" -}}
app.kubernetes.io/name: {{ include "superset-service.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
app: superset
{{- end }}

{{/*
fix-routing init container — MacVLAN + Envoy probe asymmetric routing fix.
*/}}
{{- define "superset-service.fixRouting" -}}
- name: fix-routing
...
*/}}
{{- define "superset-service.consulTagPatcher" -}}
{{- $nadName := .nadName -}}
{{- $port    := .port    -}}
{{- $ctx     := .ctx     -}}
- name: consul-tag-patcher
  ...
  command:
    ...
    - |
      ...
      pod_port    = os.environ.get("POD_PORT", "8088")
      ...
      patch = json.dumps({"metadata": {"annotations": {
          ...
          "consul.hashicorp.com/service-meta-role":        "superset",
      }}}).encode()
